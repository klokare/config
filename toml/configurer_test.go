...
func TestConfigureFailedFile(t *testing.T) {
	_, err := NewFromFile("boaty-mcboatface.toml")
	if err == nil {
...
func TestConfigure(t *testing.T) {
	...
	var tests = []struct {
		...
		// Nested, no section
		{
			Source:        []byte(`Name = "Hemmingway"`),
			Item:          &Nested{},
			Expected:      &Nested{}, // TOML does not support nested, named properties outside of section
			IsConfigError: false,
		},
		// Nested with section
		{
			Source:        []byte("[Data]\nName = \"Hemmingway\""),
			Item:          &Nested{},
			Expected:      &Nested{Data: Simple{Name: "Hemmingway"}},
			IsConfigError: false,
		},
		...
		// Nested with anonymous member in section
		{
			Source:        []byte("[Simple]\nName = \"Hemmingway\""),
			Item:          &NestedAnonymous{},
			Expected:      &NestedAnonymous{}, // TOML does not support anonymous properties as a named section
			IsConfigError: false,
		},
		// Simple in named section
		{
			Source:        []byte("[Simple]\nName = \"Hemmingway\""),
			Item:          &Simple{},
			Expected:      &Simple{}, // TOML does not support top-level structs as a named section
			IsConfigError: false,
		},
	}
